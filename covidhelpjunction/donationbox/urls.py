...
urlpatterns = [
    ...
    path('login/',auth_views.LoginView.as_view(template_name="donationbox/login.html"),name='login'), #here in as_view() we are providing the path to django where to look for the template 
    path('logout/',auth_views.LogoutView.as_view(template_name="donationbox/logout.html"),name='logout'), #here in as_view() we are providing the path to django where to look for the template
    # for logout by default django provide us but we can change that template as stated earlier.
    path('password-reset/',auth_views.PasswordResetView.as_view(template_name="donationbox/password_reset.html"),name='password_reset'),
    path('password-reset/done/',auth_views.PasswordResetDoneView.as_view(template_name="donationbox/password_reset_done.html"),name='password_reset_done'), #  this is notify that we are reseting password and one mail is sent to your mail containing the link to reset
    path('password-reset-confirm/<uidb64>/<token>/',auth_views.PasswordResetConfirmView.as_view(template_name="donationbox/password_reset_confirm.html"),name='password_reset_confirm'), # this is the url that django expect from our side otherwise it will show an error after putting the email in the input bar i.e Reverse for 'password_reset_confirm' not found. 'password_reset_confirm' is not a valid view function or pattern name.
    path('password-reset-complete/',auth_views.PasswordResetCompleteView.as_view(template_name="donationbox/password_reset_complete.html"),name='password_reset_complete'),
